...
const TVShowsAiringTodayNew = () => {
  ...
  const [showOnlyFirstTime, setShowOnlyFirstTime] = useState(false);
  const [showOnlyThisMonth, setShowOnlyThisMonth] = useState(false);
  const [showOnlyThisYear, setShowOnlyThisYear] = useState(false);
  ...
  const displayShowsRaw = selectedPlatform ? showsWithProviders : shows;
  const thisYearShows = displayShowsRaw.filter(show =>
    show.first_air_date >= firstDayOfYear && show.first_air_date <= lastDayOfYear
  );
  const displayShows = showOnlyThisYear ? thisYearShows : displayShowsRaw;

  if (loading) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        <h2>Loading Airing Today TV Shows...</h2>
      </div>
    );
  }
...
export default TVShowsAiringTodayNew;